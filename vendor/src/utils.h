...
void select_seeds(graph& G, parlay::sequence<parlay::sequence<vertex>>& seeds, size_t num_batch, size_t set_size){
  ...
  // 2) keep only those vertices whose degree ≥ set_size
  ...
  // 3) build the inverse map: get_order[u] = position of u in `orders`
  //    so we can randomly sort neighbors by that same permutation
  parlay::sequence<vertex> get_order(n);
  parlay::parallel_for(0, n, [&](size_t i){
    get_order[orders[i]]=i;
  });

  // 4) for each candidate center v, build one seed-set
  for (auto v : vertices){
    // if (G[v].size() < set_size) continue;
    seeds[r][0]=v;
    size_t ns=1;

    // 5) sort v’s neighbors in the same random order
    auto neighbors = parlay::sort(G[v], [&](vertex a, vertex b){
      return get_order[a]<get_order[b];});

    // 6) take the first (set_size−1) distinct neighbors
    ...
    // 7) if v had fewer than (set_size−1) neighbors, pad with v
    for (;ns<set_size;ns++){
      seeds[r][ns]=v;
    }
    r++;
